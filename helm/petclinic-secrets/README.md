...
The base chart keeps both ExternalSecrets disabled and leaves remote secret
names empty. Environment-specific values in `helm-values/secrets-dev.yaml` and
`helm-values/secrets-prod.yaml` enable them and set the remote secret names.
