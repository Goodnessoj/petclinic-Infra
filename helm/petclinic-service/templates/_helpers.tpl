...
image:
  ...
  repository: petclinic-prod-config-server
  repositoryPrefix: petclinic-prod
  tag: 64a3a3d
  pullPolicy: IfNotPresent

Final image:
123456789012.dkr.ecr.us-east-2.amazonaws.com/petclinic-prod-config-server:64a3a3d
*/}}
...
{{- $repository := .Values.image.repository | default "" | trimPrefix "/" | trimSuffix ":" -}}
{{- if not $repository -}}
{{- $repositoryPrefix := required "image.repository or image.repositoryPrefix is required. Example: petclinic-prod." .Values.image.repositoryPrefix | trimSuffix "-" -}}
{{- $repository = printf "%s-%s" $repositoryPrefix (include "petclinic-service.name" .) -}}
{{- end -}}
